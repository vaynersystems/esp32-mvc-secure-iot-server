    ...
    function ensurePathExists(path){
        ...
        for(const part of pathParts){
            ...
            if(parentElement == null)
                ...
                child.style.marginLeft = ((isNaN(marginleft) ? 0 : marginleft) + left_margin) + "px";
    ...
    function addList(treeElementId, rootPath, fileList) {
        ...
        for (var i = 0; i < fileCount; i++) {
           //ensureParentsExist(files[i].parent_dir);
            ...
            if (parentElement !== null) {
                var marginleft = isNaN(parseInt(parentElement.style.marginLeft.replace('px', ''))) ? 0 : parseInt(parentElement.style.marginLeft.replace('px', ''));
...
function createEditor(container, path, g, h, i) {
    ...
    function _addTab(path){
        var tab = {url: path, index: openTabs.length, name: path.substring(path.lastIndexOf('/'))};
        openTabs.push(tab);
